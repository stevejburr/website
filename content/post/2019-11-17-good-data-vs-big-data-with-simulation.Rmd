...
For this example, let's imagine that we can't ever measure the "group membership" which defines how the data is generated, so we cannot actually weight to it. But we do have access to another binary variable, which we know is correlated with group membership. 
